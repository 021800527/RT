...
def overlay_rss_on_building(rss_data, building_png_path, output_path, map_size):
    ...
    注意：本函数假设输入的 building_png 是由 generate_2d_map 生成的，
          其中：建筑区域 = 255（白色），非建筑区域 = 0（黑色）
    """
    ...
    epsilon = 1e-12
    total_rss_dbm = 10 * np.log10(np.where(rss_cropped == 0, epsilon, rss_cropped / 1e-3))

    # Step 4: 加载建筑图（白建筑=255，黑背景=0）
